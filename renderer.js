...
ipcRenderer.on('update-status', (event, data) => {
  switch (data.status) {
    ...
    case 'major-update':
      pendingUpdateVersion = data.version;
      updateTitle.textContent = i18n('updateAvailable');
      updateMessage.textContent = `v${data.version} ${i18n('update.majorAvailable') || 'yeni sürüm mevcut! İndirmek için tıklayın.'}`;
      updateProgress.style.display = 'none';
      downloadUpdateBtn.style.display = 'none';
      installUpdateBtn.style.display = 'none';
      laterUpdateBtn.style.display = 'block';
      ...
      // Add/replace a "Download" button that opens browser
      let openPageBtn = document.getElementById('openReleasePageBtn');
      if (!openPageBtn) {
        openPageBtn = document.createElement('button');
        openPageBtn.id = 'openReleasePageBtn';
        openPageBtn.className = downloadUpdateBtn.className;
        openPageBtn.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"></path><polyline points="15 3 21 3 21 9"></polyline><line x1="10" y1="14" x2="21" y2="3"></line></svg> <span>${i18n('update.download') || 'İndir'}</span>`;
        updateActions.insertBefore(openPageBtn, laterUpdateBtn);
      }
      openPageBtn.style.display = 'flex';
      openPageBtn.onclick = () => {
        ipcRenderer.send('open-release-page', data.url);
      };
